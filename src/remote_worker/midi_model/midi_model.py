...
class MIDIModel(PreTrainedModel):
    ...
    def forward(self, x, cache=None):
        ...
        x = x.sum(dim=-2)
        x = self.net.forward(
            inputs_embeds=x, past_key_values=cache, use_cache=cache is not None
        )
    ...
    def sample_top_p_k(self, probs, p, k, generator=None):
        ...
        next_token = torch.multinomial(
            probs_sort.reshape(-1, shape[-1]), num_samples=1, generator=generator
        ).reshape(*shape[:-1], 1)
                ...
                for i in range(max_token_seq):
                    ...
                    for b in range(batch_size):
                        ...
                        if i == 0:
                            mask[
                                b,
                                list(tokenizer.event_ids.values()) + [tokenizer.eos_id],
                            ] = 1
                        else:
